...
describe Aws::Lex::Conversation::Type::V1::Slot do
  let(:current_intent) { build(:current_intent_v1) }
  let(:name) { :test }
  ...
  describe '#details' do
    context 'when a matching key in slot_details exists' do
      ...
      it 'returns a SlotDetail instance' do
        expect(subject.details).to be_a(Aws::Lex::Conversation::Type::V1::SlotDetail)
      end
