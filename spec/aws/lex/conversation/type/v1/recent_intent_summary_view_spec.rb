...
describe Aws::Lex::Conversation::Type::V1::RecentIntentSummaryView do
  ...
  describe '#restore' do
    context 'with a Close action' do
      subject { build(:recent_intent_summary_view_v1, :close) }
    ...
    context 'with a ConfirmIntent action' do
      subject { build(:recent_intent_summary_view_v1, :confirm_intent) }
    ...
    context 'with a Delegate action' do
      subject { build(:recent_intent_summary_view_v1, :delegate) }
    ...
    context 'with an ElicitIntent action' do
      subject { build(:recent_intent_summary_view_v1, :elicit_intent) }
    ...
    context 'with an ElicitSlot action' do
      subject { build(:recent_intent_summary_view_v1, :elicit_slot) }
    ...
    context 'with an invalid DialogActionType' do
      subject { build(:recent_intent_summary_view_v1, :invalid) }
